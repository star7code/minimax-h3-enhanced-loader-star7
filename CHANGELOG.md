...
- 通用工作流将旧 FP16 修复加载器替换为增强载入节点；VSA 工作流同步使用
  最新增强载入节点与结构，保留原有生成参数和连线。
- README 增加 FastH3 Dense、FastH3 VSA DataFree 的 Hugging Face 单文件下载
  地址及备用网盘地址。

## 1.2.5 - 2026-08-31

- SM80+ 的官方、第三方及 FastH3 模型始终在创建阶段显式采用 BF16；检测到
  启动器全局 `--fp16-unet` 时仅对 H3 自动纠正回原生 BF16，并在日志说明。
- 量化模型继续保持 MixedPrecisionOps 与 INT8/ConvRot 原生分发；明确需要
  受保护 FP16 时使用独立的 Native FP16 Loader。
