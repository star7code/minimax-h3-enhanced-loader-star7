...
#
# The original six-way autotune is prohibitively expensive on the RTX 2080 Ti:
# Triton benchmarks every candidate on the first full video sequence before the
# first sampling step can finish. FastH3 VSA is inference-only here, so use one
# conservative configuration and cache it. This removes the multi-minute first
# step without changing the trained VSA routing or tensor contract.
configs = [
...
def _attn_fwd_sparse(
    ...
    for i in range(0, kv_blocks):
        ...
        v = tl.load(V_ptr)
        # Match the value tensor dtype. The upstream kernel hard-coded BF16
        # probabilities, but SM75 ComfyUI dispatch supplies FP16 V tensors and
